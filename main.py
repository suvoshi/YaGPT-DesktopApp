...
def main(page):
    ...
    send_iconbutton = ft.IconButton(icon=ft.icons.ARROW_CIRCLE_RIGHT, icon_color=PALETTE["light"], icon_size=59, on_click=send_click)
    ...
    send_textfield_container = ft.Container(
        content=send_textfield,
        margin=ft.margin.only(top=14, left=10, bottom=10)
    )

    send_panel = ft.Row(
        [
            send_textfield_container, send_iconbutton
        ], spacing=0
    )
